...
def parse_args():
    ...
    p.add_argument('--ipo-tracker',     action='store_true', dest='ipo_tracker',     help='IPO Tracker + Unlock Calendar')
    ...
    if unlocked:
        ...
        for e in unlocked[:20]:
            ...
            t3.add_row(
                ...
                e['list_date'],
                str(e['unlock_date']),
                f"[{ret_color}]{'+' if e['pct_from_list'] >= 0 else ''}{e['pct_from_list']:.0f}%[/{ret_color}]",
                f"{e['pct_from_ath']:.0f}%",
                f"Rs {e['cur']:,.0f}",
            )
        console.print(t3)
...
def main():
    ...
    if args.legend:
        ...
        return
    elif getattr(args, 'ipo_tracker', False):
        analyze_ipo_tracker()
        return
...
def analyze_value(filter_sector=None):
    ...
    console.print('[dim]Score: PB 40pts + Float 20pts + ROE 20pts + Earnings Growth 20pts. Green â‰¥65, Yellow â‰¥46.[/]') # VALUE_SCORE_PATCHED
