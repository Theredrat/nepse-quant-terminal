...
# â”€â”€ SECTOR MAP (auto-generated from nepse_market_data.db) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
SECTOR_MAP = {
    "Commercial Banks": [
...
@contextmanager
def get_db(path="nepse_market_data.db"):
    import sqlite3
...
# â”€â”€ NEPSE CLIENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ POWER SELL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_power_sell(full_df, live_df):
    ...
    if full_df is None or full_df.empty:
        try:
            import sqlite3 as _sq6, pandas as _pd6
            _c6 = _sq6.connect("nepse_market_data.db")
            _today = _c6.execute("SELECT MAX(date) FROM broker_activity").fetchone()[0]
            _r6 = _c6.execute("SELECT symbol, broker_id, buy_qty, sell_qty FROM broker_activity WHERE date=? AND (buy_qty>0 OR sell_qty>0)", (_today,)).fetchall()
            _c6.close()
            if _r6:
                rows = []
                for sym,bid,bq,sq in _r6:
                    if bq>0: rows.append({"symbol":sym,"buyer_broker":bid,"seller_broker":"","quantity":bq})
                    if sq>0: rows.append({"symbol":sym,"buyer_broker":"","seller_broker":bid,"quantity":sq})
                full_df = _pd6.DataFrame(rows)
            else:
                console.print("[red]No floorsheet data.[/red]"); return []
        except Exception as _e6:
            console.print(f"[red]No floorsheet: {_e6}[/red]"); return []
    ...
    for r in results[:15]:
        ...
        t.add_row(
            ...
            f"{r['net_sold']:,}",
            sig,
        )  # Symbol + LTP columns already included above
    console.print(t)
    return results

# â”€â”€ SECTOR ROTATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€



# â”€â”€ SECTOR MOMENTUM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _momentum_label(r5, r10, r20):
    ...
    accelerating = (r5 is not None and r10 is not None and r5 > 0 and r5 > r10)
...
def analyze_sector_trend(db_path="nepse_market_data.db"):
    ...
    if top[1].get(5) is not None:
        ...
        console.print("  [dim]Strongest: Insufficient data[/dim]")
    if bot[1].get(5) is not None:
        console.print(f"  [red]Weakest:  [/red] {bot[0]}  ({bot[1].get(5):+.2f}% / 5d)")
    else:
        console.print("  [dim]Weakest: Insufficient data[/dim]")
    console.print()
...
def analyze_sector_rotation(full_df, live_df):
    ...
    # Fallback: calculate from DB if live API returns 0 (after market close)
    db_chg = {}
    try:
        import sqlite3
        conn = sqlite3.connect('nepse_market_data.db')
        rows = conn.execute("""
            SELECT t.symbol, t.close, y.close as prev_close
            FROM stock_prices t
            JOIN stock_prices y ON t.symbol = y.symbol
            WHERE t.date = (SELECT MAX(date) FROM stock_prices)
            AND y.date = (SELECT MAX(date) FROM stock_prices WHERE date < (SELECT MAX(date) FROM stock_prices))
        """).fetchall()
        conn.close()
        for sym, close, prev in rows:
            if prev and prev > 0:
                db_chg[sym] = ((close - prev) / prev) * 100
    except Exception as _e:
        pass  # silenced: {_e}

    # Build ldf from live or DB fallback
    if (live_df is None or live_df.empty) and db_chg:
        import pandas as pd
        ldf = pd.DataFrame([{"symbol": k, "change_pct": v} for k, v in db_chg.items()])
    elif live_df is not None and not live_df.empty:
        ldf = live_df.copy()
        if "change_pct" in ldf.columns and ldf["change_pct"].abs().sum() == 0 and db_chg:
            ldf["change_pct"] = ldf["symbol"].map(db_chg).fillna(0)
    else:
        ldf = None
...
def analyze_whales(full_df):
    ...
    for sym, grp in full_df.groupby('symbol'):
        ...
        for action, col in [('BUY','buyer_broker'), ('SELL','seller_broker')]:
            ...
            min_value = 5_000_000  # Rs 5M minimum to qualify as whale
        if share >= 40 and top_qty >= 1000 and amt >= min_value:
                moves.append({
...
# â”€â”€ BROKER TRACKER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_broker_tracker(full_df, live_df, broker_id):
    ...
    buy_grp  = buys.groupby('symbol').agg(buy_qty=('quantity','sum'), buy_amt=('amount','sum'))
    sell_grp = sells.groupby('symbol').agg(sell_qty=('quantity','sum'), sell_amt=('amount','sum'))
    merged   = buy_grp.join(sell_grp, how='outer').fillna(0)
    merged['net_qty'] = (merged['buy_qty'] - merged['sell_qty']).astype(int)
    merged['net_amt'] = merged['buy_amt'] - merged['sell_amt']
    merged['buy_qty'] = merged['buy_qty'].astype(int)
    merged['sell_qty'] = merged['sell_qty'].astype(int)
    net_rows = merged.reset_index().rename(columns={'index':'symbol'}).to_dict('records')
...
# â”€â”€ SUPPORT / RESISTANCE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_support_resistance(full_df, symbol):
    ...
    console.print(Panel(
        ...
        f"[dim]Note: Based on today's intraday data only. Use with historical price context.[/dim]",
...
def analyze_watchlist(live_df):
    ...
    console.print()
    if live_df is None or live_df.empty or ("change_pct" in getattr(live_df,"columns",[]) and live_df["change_pct"].abs().sum()==0):
        try:
            import sqlite3 as _sq5, pandas as _pd5
            _c5 = _sq5.connect("nepse_market_data.db")
            _r5 = _c5.execute("""
                SELECT t.symbol, t.close, t.open, t.high, t.low, t.volume,
                       ((t.close-y.close)/y.close)*100, y.close,
                       MAX(h.high), MIN(h.low), t.close*t.volume
                FROM stock_prices t JOIN stock_prices y ON t.symbol=y.symbol
                JOIN stock_prices h ON t.symbol=h.symbol
                WHERE t.date=(SELECT MAX(date) FROM stock_prices)
                AND y.date=(SELECT MAX(date) FROM stock_prices WHERE date<(SELECT MAX(date) FROM stock_prices))
                AND h.date>=date((SELECT MAX(date) FROM stock_prices),'-365 days')
                GROUP BY t.symbol
            """).fetchall()
            _c5.close()
            if _r5:
                live_df = _pd5.DataFrame(_r5, columns=["symbol","ltp","open","high","low","volume","change_pct","prev_close","week52_high","week52_low","turnover"])
            else:
                console.print("[red]No price data.[/red]"); return
        except Exception as _e5:
            console.print(f"[red]DB error: {_e5}[/red]"); return
...
# â”€â”€ SIGNAL ENGINES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def signal_volume_breakout(df, db_path="nepse_market_data.db"):
    if 'volume' not in df.columns: return pd.DataFrame()
    r = df[df['volume'].notna() & df['ltp'].notna() & (df['volume'] > 0)].copy()
    # Load each stock's own 20-day avg volume from DB
    own_avg = {}
    try:
        import sqlite3 as _sv
        conn = _sv.connect(db_path)
        rows = conn.execute("""
            SELECT symbol, AVG(volume) FROM stock_prices
            WHERE date >= date('now', '-20 days') GROUP BY symbol
        """).fetchall()
        conn.close()
        own_avg = {s: v for s, v in rows if v}
    except Exception as _e:
        pass
    market_med = r['volume'].median()
    def vol_base(sym):
        return own_avg.get(sym, market_med) or market_med
    r['vol_ratio'] = r.apply(lambda x: x['volume'] / max(vol_base(x['symbol']), 1), axis=1)
    r = r[(r['vol_ratio'] >= 2.0) & (r['change_pct'] > 0)].copy()
    r['signal']   = 'VOLUME BREAKOUT'
    r['strength'] = (r['vol_ratio'] * r['change_pct']).round(2)
    r['reason']   = r.apply(lambda x: f"Vol {x['vol_ratio']:.1f}x own avg | +{x['change_pct']:.2f}%", axis=1)
...
# â”€â”€ QUICK PICK â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _load_latest_prices_from_db(db_path="nepse_market_data.db"):
    ...
    try:
        ...
        rows = conn.execute("""
            SELECT t.symbol, t.close, t.open, t.high, t.low, t.volume,
                   ...
                   MAX(h.high) as week52_high, MIN(h.low) as week52_low,
        ...
        conn.close()
        if rows:
            return _pd.DataFrame(rows, columns=['symbol','ltp','open','high','low','volume','change_pct','prev_close','week52_high','week52_low','turnover'])
    except Exception as e:
        pass
    return None
...
def analyze_quick_pick(live_df, top_n=10, db_path="nepse_market_data.db"):
    ...
    if live_df is None or live_df.empty:
        try:
            ...
            conn2.close()
            if rows2:
                live_df = pd.DataFrame(rows2, columns=['symbol','ltp','open','high','low','volume','change_pct','prev_close','week52_high','week52_low','turnover'])
                console.print("[dim](Using latest DB prices - after market close)[/dim]")
            else:
                console.print("[red]No live data.[/red]"); return []
        except Exception as e:
            console.print(f"[red]No live data: {e}[/red]"); return []
    df = live_df.copy()
    df = df[df["ltp"].notna() & df["volume"].notna() & (df["ltp"] > 0)].copy()
    # DB fallback if live data has no change_pct (after market close)
    if "change_pct" not in live_df.columns or live_df["change_pct"].abs().sum() == 0:
        try:
            ...
            conn2.close()
            if rows2:
                df = _pd2.DataFrame(rows2, columns=['symbol','ltp','open','high','low','volume','change_pct','prev_close','week52_high','week52_low','turnover'])
                console.print("[dim](Using latest DB prices - after market close)[/dim]")
        except Exception as _e:
            pass  # silenced: {_e}
    ...
    t.add_column("Signals",  min_width=30)
    for rank, r in enumerate(scores[:top_n], 1):
        chg_col = "green" if r["change"] >= 0 else "red"
        t.add_row(
            str(rank),
            r["symbol"],
            f"Rs {r['ltp']:,.1f}",
            f"[{chg_col}]{r['change']:+.2f}%[/{chg_col}]",
            f"[bold green]{r['score']}[/bold green]",
            f"{r['upside']:+.1f}%",
            f"[dim]{r['reasons']}[/dim]",
        )
    console.print(t)
    console.print(f"[dim]Scored {len(scores)} candidates. Top {min(top_n,len(scores))} shown.[/dim]")
    return scores
...
def analyze_smart_pick(live_df, full_df, top_n=10):
    ...
    import sqlite3 as _sq4, pandas as _pd4
    if live_df is None or live_df.empty or ("change_pct" in getattr(live_df, "columns", []) and live_df["change_pct"].abs().sum() == 0):
        try:
            _c4 = _sq4.connect("nepse_market_data.db")
            _r4 = _c4.execute("""
                SELECT t.symbol, t.close, t.open, t.high, t.low, t.volume,
                       ((t.close-y.close)/y.close)*100, y.close,
                       MAX(h.high), MIN(h.low), t.close*t.volume
            ...
            _c4.close()
            if _r4:
                live_df = _pd4.DataFrame(_r4, columns=["symbol","ltp","open","high","low","volume","change_pct","prev_close","week52_high","week52_low","turnover"])
            else:
                console.print("[red]No price data available.[/red]"); return
        except Exception as _e4:
            console.print(f"[red]DB error: {_e4}[/red]"); return

    # Start with quick pick scores as base
    quick_scores = analyze_quick_pick(None, top_n=50)
    if not quick_scores:
        return
...
def parse_args():
    ...
    p.add_argument("--offline", action="store_true", help="Use DB only, skip API calls")
    return p.parse_args()
...
def analyze_portfolio(symbols):
    ...
    console = Console()
    console.print()
    console.print(f"[bold cyan]{'â”€'*30} Portfolio Intelligence {'â”€'*30}[/bold cyan]")
    ...
    # â”€â”€ Price data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Volatility & stats per symbol â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    import numpy as np
    stats = {}
    for sym in symbols:
        r = returns_map[sym]
    ...
    # â”€â”€ Inverse-volatility weights â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    inv_vols = {s: 1 / max(stats[s]['vol'], 0.1) for s in symbols}
    total_inv = sum(inv_vols.values())
    weights = {s: inv_vols[s] / total_inv * 100 for s in symbols}

    # â”€â”€ Correlation matrix (top 20 by weight only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Table 1: Per-stock stats + suggested weight â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Diversification score â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if div_score >= 70:
        div_color = "green"
...
def analyze_corr():
    ...
    console = Console()
    console.print()
    console.print(f"[bold cyan]'-- Sector Correlation Heatmap ' + '-'*28[/bold cyan]")
    ...
    console.print("  [red][RED] Red  > 0.85[/red]  - Move together always    -> Avoid holding both, no diversification")
    ...
    if pairs_green:
        ...
        console.print("  [yellow]â ï¸  No green pairs in NEPSE - all sectors are correlated[/yellow]")
    ...
    console.print()
    console.print("  [red][X] Avoid holding together (move in sync):[/red]")
...
def analyze_size(symbol, capital):
    ...
    symbol = symbol.upper()
    console.print()
    console.print(f"[bold cyan]{'â”€'*30} Position Sizer: {symbol} {'â”€'*30}[/bold cyan]")
...
def main():
    ...
    live_df = None
    if need_live and not getattr(args, "offline", False):
    ...
    full_fs = None
    if need_floor and not getattr(args, "offline", False):
...
# â”€â”€ RELATIVE STRENGTH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_relative_strength():
    ...
    # â”€â”€ TOP OUTPERFORMERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ SECTOR SUMMARY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    import pandas as pd
    df = pd.DataFrame(data)
    sec_summary = (
        df.groupby("sector")
    ...
    # â”€â”€ BOTTOM LAGGARDS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if bot:
        btbl = Table(
...
# â”€â”€ BROKER ACCUMULATION on RS LEADERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_broker_rs():
    ...
    # â”€â”€ Step 3: Broker net activity per RS stock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    results = []
    ...
    # â”€â”€ Step 4: Top accumulators (net buyers in RS leaders) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Step 5: Broker conviction table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Step 6: RS stocks with no broker accumulation (warning) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if not_accumulated:
        console.print(f"  [yellow]â  RS leaders with NO net broker buying:[/] {', '.join(sorted(not_accumulated))}")
...
# â”€â”€ 52-WEEK HIGH/LOW ALERTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_week52(db_path="nepse_market_data.db"):
    ...
    console.print("[dim]Breakout candidates near highs Â· Recovery plays near lows Â· Cross-checked with RS[/]\n")

    conn = sqlite3.connect(db_path)

    # â”€â”€ Pull 52-week range + current price for all equity stocks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    df = pd.read_sql_query("""
        SELECT
            sp.symbol,
            c.sector,
    ...
    # â”€â”€ Get RS data for cross-reference â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    rs_data   = _calc_relative_strength()
    rs_lookup = {r["symbol"]: r["rs5"] for r in rs_data} if rs_data else {}

    df["rs5"] = df["symbol"].map(rs_lookup).fillna(0)

    # â”€â”€ Near 52-week HIGH (within 5%) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Near 52-week LOW (within 10%) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Sector summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Top conviction: near high + strong RS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    conviction = df[
...
# â”€â”€ --fundamental SYMBOL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ --earnings SYMBOL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ --value  (peer comparison by sector) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def analyze_value(filter_sector=None):
    ...
    # â”€â”€ Pre-load earnings growth map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    for sec in sorted(sectors):
        ...
        for _rank_idx, (_row_i, r) in enumerate(sec_df.iterrows(), 1):
            ...
            # â”€â”€ Earnings growth score (QoQ) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            # â”€â”€ Value Score 0-100 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            score = 0
            ...
            # â”€â”€ Verdict â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            tags = []
    ...
    if all_scores:
        ...
        lb = _T(title='[bold yellow]â˜-> Top 10 Value Picks - All Sectors[/]',
...
# â”€â”€ --float SYMBOL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def analyze_why(live_df, full_fs, rs_data=None, db_path="nepse_market_data.db"):
    ...
    # â”€â”€ WHY BLOCK â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def log_broker_activity(fs_df, db_path='nepse_market_data.db'):
    try:
        ...
        for _, row in merged.iterrows():
            ...
            bid = bid_raw if bid_raw.isdigit() else bid_raw
...
def analyze_broker_impact(days=30, top_n=20, db_path="nepse_market_data.db"):
    ...
    try:
        ...
        for rank, r in enumerate(scored, 1):
            ...
            t.add_row(
                str(rank), str(bid), (bname or str(bid))[:28],
                str(stocks), str(apps),
                _fmt_rs_val(avg_size or 0),
                _fmt_rs_val(tbought or 0),
                _fmt_rs_val(tsold or 0),
                f"[{bc}]{bias}[/{bc}]"
            )
        console.print(t)
        console.print()
        ...
        for r in buyers:
            console.print(f"  [green]Broker {r[0]} ({(r[1] or str(r[0]))[:25]}) - avg {_fmt_rs_val(r[8] or 0)} per trade, {r[2]} stocks[/green]")
        console.print()
        console.print("[bold]Top 5 Institutional Sellers:[/bold]")
        sellers = [r for r in scored if r[10] == "SELLER"][:5]
        for r in sellers:
            console.print(f"  [red]Broker {r[0]} ({(r[1] or str(r[0]))[:25]}) - avg {_fmt_rs_val(r[8] or 0)} per trade, {r[2]} stocks[/red]")
        console.print()
    except Exception as e:
        console.print(f"  Error: {e}", style="red")
